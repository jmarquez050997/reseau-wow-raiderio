# Réseau World of Warcraft mythique+
...
Le choix par défaut au premier lancement est Europe pour la région et 20 pour le nombre d'instance. À l'initialisation de la page ainsi qu'à chaque changement dans l'un des deux sélecteurs, les données sont automatiquement récupérées et actualisées grâce à l'envoi des requêtes API adéquates. Le chargement de la visualisation peut avoir une latence de 1-2 secondes selon sa taille.

## À propos du Réseau World of Warcraft mythique+
La visualisation en réseau des donjons mythiques+ de World of Warcraft offre un premier outil d'exploration dans cet écosystème gargantuesque. L'aspect social dans les MMORPG est une composante essentielle, mais il était intéressant de se demander si des liens pouvaient exister entre les meilleurs joueurs. En tant que joueur, l'image qu'on se fait de ce cercle est cloisonnée. L'idée dominante est que les meilleurs joueurs forment des équipes et ne compétitionne qu'avec ce groupe. Cette visualisation permet de prendre du recul face à cette représentation et de se rendre compte que la situation est bien différente en fonction des régions. Certains groupes restent effectivement avec leur équipe, mais, on constate que plus on augmente le nombre d'instances, plus un véritable réseau en parallèle se forme. Le classement des meilleurs donjons de raider.io ne permettait pas de faire ce constat aussi clairement.

Cependant, certains points sont absents de cette visualisation. Tout d'abord, le classement des meilleurs donjons n'est pas pris en compte. Il s'agit uniquement d'un critère de sélection, mais pas d'un aspect visible dans le réseau. Les joueurs ayant les meilleurs donjons ne se démarquent pas des autres et ne sont qu'un point parmi d'autres. De plus, les informations sur les donjons, comme le temps ou le niveau ne sont pas considérés. Cette omission amène un autre constat, les nombres de fois qu'un même groupe apparait dans les données n'est pas considéré, notamment pour des questions de lisibilité du réseau et aussi pour ne pas empiéter sur les fonctionnalités d'exploration. Ce réseau n'examine que les meilleurs donjons réussis et non les tentatives ou les échecs. Ce réseau n'est qu'une partie de la réalité de ce mode de jeu, en ne montrant que l'excellence de la réussite et non les nombreuses tentatives sous-jacentes.
